...
## 2026-03-04 - [Config Drift Fixes] **Bottleneck:** Missing `.pem` in `.gitignore` exposing keys, and missing chunk warning limit threshold causing CI output noise. **Fix:** Added `*.pem` to `.gitignore` to prevent committing secrets and bumped `chunkSizeWarningLimit` to `1000` in `vite.config.ts` to ignore expected large files like `ort-wasm`.
