...
## 2026-03-27 - [Strict Infrastructure Versions] **Bottleneck:** Lack of strict dependency versions allowed drift (dynamic pnpm in GitHub Actions and generic nginx:alpine Docker image). **Fix:** Enforced dynamic extraction of pnpm version from `package.json` in the setup action using shell tools (`grep`/`cut`) to avoid implicit Node.js dependencies, and pinned Dockerfile nginx stage to strict patch version `1.27.3-alpine`.
