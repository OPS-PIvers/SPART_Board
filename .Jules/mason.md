...
## 2026-03-07 - [CI Duplication & Ineffective Caching] **Bottleneck:** Workflows duplicated Node/pnpm setup across 9 jobs, and Playwright caching was missing the `if` condition, causing browsers to re-download on every PR. **Fix:** Consolidated setup into a local composite action (`.github/actions/setup`) and added cache-hit conditions to Playwright install steps.
