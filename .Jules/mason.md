...
## 2026-02-04 - [CI Standardization] **Bottleneck:** Config drift in dev deployments and inconsistent install steps across workflows. **Fix:** Standardized `firebase-dev-deploy.yml` to use `dev-*` wildcard and caching; consolidated install steps in `firebase-deploy.yml` to use `pnpm run install:ci`.
