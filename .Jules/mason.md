...
## 2026-03-27 - [Strict Infrastructure Versions] **Bottleneck:** Lack of strict dependency versions allowed drift (dynamic pnpm in GitHub Actions and generic nginx:alpine Docker image). **Fix:** Enforced dynamic extraction of pnpm version using `jq` from `package.json` in setup action to avoid implicit node dependencies, and pinned Dockerfile nginx stage to strict patch version `1.27.3-alpine`.
