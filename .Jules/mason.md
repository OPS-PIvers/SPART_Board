...
## 2026-02-10 - [Simplified Deployment Workflow] **Bottleneck:** Redundant manual steps in `firebase-deploy.yml` caused maintenance overhead and inconsistency with `package.json` scripts. **Fix:** Refactored workflow to use unified `install:ci` and `build:all` scripts, streamlining the pipeline while preserving quality gates.
