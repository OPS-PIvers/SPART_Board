...
## 2026-03-12 - [Strict Versioning and Cleanup] **Bottleneck:** Loose pnpm version constraint in CI led to broken builds, and .gitignore concatenated entries. **Fix:** Pinned `pnpm` to version 9 in `.github/actions/setup/action.yml`, aligned root `package.json`, and separated `.gitignore` entries.

## 2026-03-12 - [Conflicting PNPM versions] **Bottleneck:** Setting both `version` input in GitHub Actions and `packageManager` field in `package.json` causes a strict version conflict (`ERR_PNPM_BAD_PM_VERSION`) breaking all CI checks. **Fix:** Removed `version` input in `.github/actions/setup/action.yml` to rely entirely on `package.json`.
