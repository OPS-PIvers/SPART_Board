...
## 2026-04-02 - [Redundant CI Build] **Bottleneck:** The PR validation pipeline runs a redundant 'build' job alongside the 'e2e' job, which also builds the project, wasting pipeline minutes. **Fix:** Removed the redundant 'build' job from 'pr-validation.yml' since E2E implicitly verifies the build.
