// D3 sticky bar under the progress header: Read question, speed pill (D6), Auto-read (D14).
import React from 'react';
import { useTranslation } from 'react-i18next';
import { Loader2, Square, Volume2 } from 'lucide-react';
import type { QuizReadAloudController } from './useQuizReadAloud';

export const ReadAloudToolbar: React.FC<{
  ...
  return (
    <div className="sticky top-0 z-10 border-b border-slate-200 bg-white/85 backdrop-blur">
      ...
      {message && (
        <p
          role="status"
          className="mx-auto w-full max-w-7xl px-4 pb-2 text-xs font-medium text-brand-red-primary"
        >
          {message}
        </p>
      )}
