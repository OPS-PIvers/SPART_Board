...
interface WebcamConfigurationPanelProps {
  config: WebcamGlobalConfig;
  onChange: (newConfig: WebcamGlobalConfig) => void;
}

export const WebcamConfigurationPanel: React.FC<
  WebcamConfigurationPanelProps
> = ({ config, onChange }) => {
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="flex bg-white rounded-lg border border-slate-200 p-1">
          <button
            onClick={() => onChange({ ...config, ocrMode: 'standard' })}
          ...
          <button
            onClick={() => onChange({ ...config, ocrMode: 'gemini' })}
            className={`flex-1 py-1.5 text-xxs font-bold rounded transition-colors ${
