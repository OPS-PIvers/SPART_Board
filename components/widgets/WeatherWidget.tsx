...
import { Sun, Cloud, CloudRain, CloudSnow, Wind, Thermometer, Palette, MapPin, RefreshCw, Key, AlertCircle } from 'lucide-react';
...
export const WeatherWidget: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  ...
  const getIcon = () => {
    switch(condition.toLowerCase()) {
      case 'cloudy':
      case 'clouds': return <Cloud className="w-12 h-12 text-slate-400" />;
      case 'rainy':
      case 'rain':
      ...
      case 'windy':
      case 'squall':
      case 'tornado': return <Wind className="w-12 h-12 text-slate-500" />;
      case 'sunny':
      case 'clear': return <Sun className="w-12 h-12 text-amber-400 animate-spin-slow" />;
      default: return <Sun className="w-12 h-12 text-amber-400 animate-spin-slow" />;
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-4 gap-2">
      ...
      <div className="flex items-center gap-4">
        {getIcon()}
        <div className="text-4xl font-black text-slate-800 tabular-nums">{Math.round(temp)}°</div>
      </div>

      <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mt-2">Instruction</div>
      ...
      <div className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-3 flex items-center gap-3">
        ...
        <div className="text-xs font-bold text-slate-700 leading-tight">
          Today is <span className="text-indigo-600 uppercase">{condition}</span>.<br/>
...
export const WeatherSettings: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  ...
  const fetchWeather = async (params: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?${params}&appid=${cleanKey}&units=imperial`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex bg-slate-100 p-1 rounded-xl">
        <button 
          onClick={() => updateWidget(widget.id, { config: { ...widget.config, isAuto: false } })}
        ...
        <button 
          onClick={() => updateWidget(widget.id, { config: { ...widget.config, isAuto: true } })}
          className={`flex-1 py-1.5 text-[10px] font-black rounded-lg transition-all ${isAuto ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}
        >
          AUTOMATIC
        </button>
          ...
          <div>
            ...
            <div className="flex items-center gap-4 px-2">
              <input 
                ...
                value={temp}
                onChange={(e) => updateWidget(widget.id, { config: { ...widget.config, temp: parseInt(e.target.value), locationName: 'Manual Mode' } })}
                className="flex-1 accent-indigo-600 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer"
              />
              <span className="w-10 text-center font-mono font-bold text-slate-700 text-sm">{Math.round(temp)}°</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {conditions.map(c => (
                <button
                  key={c.id}
                  onClick={() => updateWidget(widget.id, { config: { ...widget.config, condition: c.id } })}
                ...
                >
                  ...
                  <span className="text-[7px] font-black uppercase">{c.id}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div>
            ...
            <input 
              ...
              value={apiKey}
              onChange={(e) => updateWidget(widget.id, { config: { ...widget.config, apiKey: e.target.value } })}
            ...
            <div className="flex gap-1.5 mt-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
              ...
              <p className="text-[8px] text-slate-500 font-bold leading-normal">
                New keys can take <span className="text-amber-600">up to 2 hours</span> to activate. If you just created yours, please wait a while before syncing.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input 
                ...
                value={city}
                onChange={(e) => updateWidget(widget.id, { config: { ...widget.config, city: e.target.value } })}
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-[8px] font-black text-slate-300 uppercase"><span className="bg-white px-2">OR</span></div>
          </div>
