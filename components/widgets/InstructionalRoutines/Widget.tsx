...
export const InstructionalRoutinesWidget: React.FC<{ widget: WidgetData }> = ({
  ...
  const launchBloomsResource = (type: 'keyWords' | 'questionStarters') => {
    ...
    let content = `<h3 class="font-black mb-2 uppercase text-slate-800">${title}</h3>`;

    if (type === 'keyWords') {
      BLOOMS_DATA.keyWords.forEach((levelGroup) => {
        content += `<h4 class="font-extrabold mt-4 mb-1 text-brand-blue-primary text-sm">${levelGroup.level}</h4><ul class="pl-5 list-disc text-slate-600 text-xs">`;
