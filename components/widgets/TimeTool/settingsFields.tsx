...
export type CustomFieldProps = { ctx: FieldCtx; updateConfig: UpdateConfig };
...
// Segmented-style radiogroup for Custom fields; FieldRenderer's label cannot target it, so it names itself.
function TimeToolRadioGroup<V extends string | number | null>({
  ...
  return (
    <div
      role="radiogroup"
      aria-label={name}
      onKeyDown={(e) => handleRadioGroupKeyDown(e, values, onSelect)}
      ...
      {options.map((option, index) => {
        ...
        return (
          <button
            ...
            className={`flex-1 flex items-center justify-center text-xs px-2 py-1.5 rounded-md transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-brand-blue-primary ${
...
const leafResolver = (ctx: FieldCtx) => {
...
export const TimeToolModeField: React.FC<CustomFieldProps> = ({
  ctx,
  updateConfig,
}) => {
...
export const TimeToolVoiceLevelField: React.FC<CustomFieldProps> = ({
  ...
  return (
    <TimeToolRadioGroup<number | null>
      ...
      value={value}
      onSelect={(level) => updateConfig({ timerEndVoiceLevel: level })}
    />
  );
};
...
export const TimeToolTrafficColorField: React.FC<CustomFieldProps> = ({
  ...
  return (
    <TimeToolRadioGroup<TrafficColor | null>
      ...
      value={value}
      onSelect={(color) => updateConfig({ timerEndTrafficColor: color })}
...
export const TimeToolNexusHints: React.FC<CustomFieldProps> = ({ ctx }) => {
  ...
  if (missing.length === 0) {
    return <p className="text-xxs text-slate-600">{leaf('allConnected')}</p>;
  }
  return (
    <ul className="flex flex-col gap-1.5" data-testid="time-tool-nexus-hints">
      {missing.map((sibling) => (
        <li
          key={sibling.type}
          className="text-xxs text-brand-blue-dark bg-brand-blue-lighter/20 border border-brand-blue-lighter/30 rounded-lg px-2.5 py-2 leading-snug"
        >
