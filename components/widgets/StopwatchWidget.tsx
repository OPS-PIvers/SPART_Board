...
import { Play, Pause, RotateCcw, Flag, Palette, Type, Layout } from 'lucide-react';
import { useDashboard } from '../../context/DashboardContext';
import { WidgetData } from '../../types';

export const StopwatchWidget: React.FC<{ widget: WidgetData }> = ({ widget }) => {
    ...
    <div className="flex flex-col items-center justify-center h-full p-4 gap-4 overflow-hidden">
      ...
      {showLaps && laps.length > 0 && (
        <div className="w-full max-h-32 overflow-y-auto border-t border-slate-100 pt-3 custom-scrollbar">
          {laps.map((l, i) => (
            <div key={i} className="flex justify-between text-xs font-mono py-1 px-2 rounded-lg hover:bg-slate-50 transition-colors">
              <span className="font-black text-slate-300 uppercase tracking-tighter">Lap {laps.length - i}</span>
...
export const StopwatchSettings: React.FC<{ widget: WidgetData }> = ({ widget }) => {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-200">
          <span className="text-[10px] font-black text-slate-700 uppercase">Show Laps</span>
          <button
            onClick={() => updateWidget(widget.id, { config: { ...config, showLaps: !config.showLaps } })}
            className={`w-10 h-5 rounded-full relative transition-colors ${config.showLaps ? 'bg-indigo-600' : 'bg-slate-300'}`}
          >
            <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${config.showLaps ? 'translate-x-5' : 'translate-x-0'}`} />
          </button>
        </div>
      </div>

      <div>
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Size Multiplier</label>
        <div className="flex items-center gap-4">
          ...
          />
          <span className="w-8 text-center font-mono font-bold text-slate-700 text-xs">{config.scaleMultiplier || 1}x</span>
        </div>
      </div>
    </div>
  );
