...
import { VolumeX, Volume1, Volume2, Users, Briefcase, Sparkles, Trash2, ArrowRight } from 'lucide-react';

export const WorkSymbolsWidget: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  ...
  const routines = [
    { id: 'ready', label: 'Get Ready', icon: Briefcase, color: 'bg-orange-500' },
  ...
  return (
    <div className="flex flex-col h-full gap-4 p-4">
      <div className="space-y-2">
        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Voice Level</label>
        <div className="grid grid-cols-4 gap-2">
          {voices.map(v => (
            <button
              key={v.id}
              onClick={() => updateWidget(widget.id, { config: { ...widget.config, voice: voice === v.id ? 'none' : v.id } })}
            ...
            >
              ...
              <span className="text-[8px] font-black uppercase whitespace-nowrap">{v.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Instructional Routine</label>
        <div className="grid grid-cols-3 gap-2">
          {routines.map(r => (
            <button
              key={r.id}
              onClick={() => updateWidget(widget.id, { config: { ...widget.config, routine: routine === r.id ? 'none' : r.id } })}
            ...
            >
              ...
              <span className="text-[8px] font-black uppercase whitespace-nowrap">{r.label}</span>
            </button>
          ))}
        </div>
      </div>
