...
export const ScoreboardSettings: React.FC<{ widget: WidgetData }> = ({
  ...
  const importFromPoll = () => {
    ...
    if (confirm('Importing from Poll will replace current teams. Continue?')) {
      ...
      addToast(`Imported ${newTeams.length} poll options!`, 'success');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex flex-col gap-3">
        ...
        {!pollWidget && (
          <div className="text-xxs text-indigo-400 font-medium">
            Tip: Add a Poll widget to import results as scores.
          </div>
        )}
      </div>
