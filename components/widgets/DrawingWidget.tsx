...
import { 
  Pencil, Eraser, Trash2, Maximize, Minimize, 
  Download, Palette, Undo2, MousePointer2 
} from 'lucide-react';
...
export const DrawingWidget: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  ...
  const draw = (ctx: CanvasRenderingContext2D, allPaths: Path[], current: Point[]) => {
  ...
  const handleEnd = () => {
    ...
    if (currentPath.length > 1) {
      ...
      updateWidget(widget.id, { config: { ...widget.config, paths: [...paths, newPath] } });
    }
    setCurrentPath([]);
  };
  ...
  const undo = () => {
    updateWidget(widget.id, { config: { ...widget.config, paths: paths.slice(0, -1) } });
  };
    ...
    <div className="flex flex-wrap items-center gap-2 p-2">
      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
        {['#1e293b', '#ef4444', '#3b82f6', '#22c55e', '#eab308'].map(c => (
          <button
            key={c}
            onClick={() => updateWidget(widget.id, { config: { ...widget.config, color: c } })}
            className={`w-6 h-6 rounded-md transition-all ${color === c ? 'scale-110 shadow-sm ring-2 ring-indigo-500' : 'hover:scale-105'}`}
            style={{ backgroundColor: c }}
          />
        ))}
        <button
          onClick={() => updateWidget(widget.id, { config: { ...widget.config, color: '#ffffff' } })}
      ...
      <button onClick={undo} title="Undo" className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 transition-colors">
        <Undo2 className="w-4 h-4" />
      </button>
      <button onClick={clear} title="Clear All" className="p-2 hover:bg-red-50 rounded-lg text-red-500 transition-colors">
      ...
      <button 
        onClick={() => updateWidget(widget.id, { config: { ...widget.config, mode: mode === 'window' ? 'overlay' : 'window' } })}
        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
          mode === 'overlay' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
        }`}
      >
        {mode === 'overlay' ? <Minimize className="w-3 h-3" /> : <Maximize className="w-3 h-3" />}
    ...
    return (
      <div className="h-full flex flex-col bg-white rounded-lg">
        <div className="flex-1 flex flex-col items-center justify-center p-4 text-center gap-3">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-slate-800">Annotation Active</p>
            <p className="text-[10px] text-slate-500 mt-1">Drawing on the entire screen.<br/>Use the controls below.</p>
          </div>
        </div>
...
export const DrawingSettings: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  const { updateWidget } = useDashboard();
      ...
      <div>
        ...
        <div className="flex items-center gap-4 px-2">
          <input 
            ...
            value={width}
            onChange={(e) => updateWidget(widget.id, { config: { ...widget.config, width: parseInt(e.target.value) } })}
            className="flex-1 accent-indigo-600 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer"
          />
          <span className="w-10 text-center font-mono font-bold text-slate-700 text-sm">{width}px</span>
        </div>
      </div>

      <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
        <h4 className="text-[10px] font-black text-indigo-700 uppercase mb-2">Modes Guide</h4>
        <div className="space-y-3">
          <div className="flex gap-3">
            ...
            <p className="text-[9px] text-indigo-600 font-medium"><b>Window:</b> Standard canvas inside the widget box. Best for quick sketches.</p>
          </div>
          <div className="flex gap-3">
            ...
            <p className="text-[9px] text-indigo-600 font-medium"><b>Overlay:</b> Annotate on top of all other widgets. Perfect for highlighting specific dashboard content.</p>
          </div>
        </div>
      </div>
    </div>
