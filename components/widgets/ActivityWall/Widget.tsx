...
export const ActivityWallWidget: React.FC<{ widget: WidgetData }> = ({
  ...
  >({});
  const submissionsGridRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const node = submissionsGridRef.current;
    if (!node) return;
  ...
  }, [showLiveView, activeActivity?.id]);
        ...
        {fullscreenSubmission && (
          <div className="rounded-2xl bg-slate-950/95 p-4 text-white">
            ...
            isLikelyVideoUrl(fullscreenSubmission.content) ? (
              <video
                src={fullscreenSubmission.content}
                controls
