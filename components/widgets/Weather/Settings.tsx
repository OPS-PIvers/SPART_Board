...
import { fetchEarthNetworksData, fetchOpenWeatherData } from './weatherService';
...
export const WeatherSettings: React.FC<{ widget: WidgetData }> = ({
  ...
  const updateWeatherConfig = (data: NormalizedWeatherData) => {
  ...
  const fetchWeather = async (params: string) => {
    ...
    try {
      ...
      const data = await fetchOpenWeatherData(systemKey, params);
