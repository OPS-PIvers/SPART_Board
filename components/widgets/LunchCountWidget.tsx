...
import { Users, Send, Coffee, Home, Box, RefreshCw, UserPlus } from 'lucide-react';
...
export const LunchCountWidget: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  ...
  const handleDrop = (e: React.DragEvent, type: LunchType) => {
    ...
    if (name) {
      ...
      updateWidget(widget.id, { config: { ...widget.config, assignments: newAssignments } });
    }
  };
  ...
  const resetCount = () => {
    if (confirm('Reset all lunch choices for today?')) {
      updateWidget(widget.id, { config: { ...widget.config, assignments: {} } });
    }
  };

  const categories: { type: LunchType; label: string; icon: any; color: string; border: string }[] = [
    { type: 'hot', label: 'Hot Lunch', icon: Coffee, color: 'bg-orange-50', border: 'border-orange-200' },
    { type: 'bento', label: 'Bento Box', icon: Box, color: 'bg-emerald-50', border: 'border-emerald-200' },
    { type: 'home', label: 'Home Lunch', icon: Home, color: 'bg-blue-50', border: 'border-blue-200' },
  ];
  ...
  if (students.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-400 p-6 text-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold uppercase tracking-widest mb-1">Class Roster Empty</p>
          <p className="text-xs">Flip this widget to enter your student names.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col p-3 bg-white gap-3 select-none">
          ...
          >
            <div className="p-2 flex items-center justify-between border-b border-dashed border-inherit bg-white/40">
              ...
              <span className="text-xs font-black text-slate-900 bg-white px-2 py-0.5 rounded-full shadow-sm">
                {students.filter((s: string) => assignments[s] === cat.type).length}
              </span>
            </div>
            <div className="flex-1 p-2 overflow-y-auto custom-scrollbar flex flex-wrap gap-1 content-start">
              {students.filter((s: string) => assignments[s] === cat.type).map((name: string) => (
                <div
                  key={name}
                  draggable
      ...
      >
        <div className="w-full text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Waiting to Choose...</div>
        {students.filter((s: string) => !assignments[s] || assignments[s] === 'none').map((name: string) => (
          <div
            key={name}
            draggable
            onDragStart={(e) => handleDragStart(e, name)}
        ...
        {students.every((s: string) => assignments[s] && assignments[s] !== 'none') && (
...
export const LunchCountSettings: React.FC<{ widget: WidgetData }> = ({ widget }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Recipient Email</label>
        <input
          type="email"
          value={recipient}
          onChange={(e) => updateWidget(widget.id, { config: { ...widget.config, recipient: e.target.value } })}
      ...
      <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl">
        <h4 className="text-[10px] font-black text-blue-700 uppercase mb-2">Instructions</h4>
        <p className="text-[9px] text-blue-600 leading-normal font-medium">
          Once students choose their lunch, click <b>"Send Lunch Report"</b>. This opens a mail composer with the final counts pre-formatted for your cafeteria staff.
        </p>
      </div>
    </div>
  );
