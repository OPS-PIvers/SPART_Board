...
export const LunchCountWidget: React.FC<{ widget: WidgetData }> = ({
  ...
  return (
    <div className="h-full flex flex-col p-3 bg-white gap-3 select-none pt-2">
      {/* Action Header Row (Slim Style) */}
      <div className="flex gap-2 shrink-0 px-1 mb-1">
        <button
          onClick={resetCount}
          className="flex items-center justify-center gap-2 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 transition-colors border border-slate-200"
        >
          <RefreshCw className="w-3 h-3" /> Reset
        </button>
        <button
          onClick={handleSend}
          className="flex-1 flex items-center justify-center gap-2 py-1.5 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm hover:bg-indigo-700 active:scale-95 transition-all"
        >
          <Send className="w-3 h-3" /> Send Lunch Report
        </button>
      </div>

      {/* Category Buckets (Dynamic Growth) */}
      <div className="grid grid-cols-3 gap-3 shrink-0">
        {categories.map((cat) => (
          <div
            ...
            className={`flex flex-col min-h-[6rem] rounded-2xl border-2 border-dashed ${cat.color} ${cat.border} transition-all relative overflow-hidden`}
          >
            <div className="p-2 flex items-center justify-between border-b border-dashed border-inherit bg-white/40">
              ...
              <span className="text-[10px] font-black text-slate-900 bg-white px-1.5 py-0.5 rounded-full shadow-sm">
                {
      ...
      {/* Waiting Area (Fills remaining space) */}
      <div
        onDragOver={(e) => e.preventDefault()}
        onDrop={(e) => {
          handleDrop(e, 'none');
