...
interface RoutineStepEditorProps {
  ...
  index: number;
  isAdmin: boolean;
  onUpdate: (index: number, newStep: RoutineStep) => void;
  onMove: (index: number, dir: 'up' | 'down') => void;
  onDelete: (index: number) => void;
}

const RoutineStepEditor = React.memo(
  ({
    step,
    index,
    isAdmin,
    ...
    return (
      <div className="flex gap-2 items-center bg-white p-3 rounded-2xl border border-slate-100 group shadow-sm">
        ...
        <div className="flex-1 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2">
                <div className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">
                  ...
                  <input
                    ...
                    value={step.label ?? ''}
                    onChange={(e) => {
                      onUpdate(index, { ...step, label: e.target.value });
                    }}
                    placeholder="Keyword"
          ...
          <textarea
            value={step.text}
            onChange={(e) => {
              onUpdate(index, { ...step, text: e.target.value });
            }}
            rows={2}
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (
                  ...
                  onUpdate(index, rest as RoutineStep);
                }
              }}
...
RoutineStepEditor.displayName = 'RoutineStepEditor';
...
export const InstructionalRoutinesSettings: React.FC<{
  ...
  // Use refs to keep callbacks stable so we don't break memoization of children
  ...
  const onUpdateStep = useCallback(
    (index: number, newStep: RoutineStep) => {
      const { config: currentConfig, widgetId } = latestState.current;
      const next = [...currentConfig.customSteps];
      next[index] = newStep;
  ...
  const onDeleteStep = useCallback(
    (index: number) => {
      const { config: currentConfig, widgetId } = latestState.current;
      const next = currentConfig.customSteps.filter((_, i) => i !== index);
      updateWidget(widgetId, {
        config: {
          ...currentConfig,
          customSteps: next,
