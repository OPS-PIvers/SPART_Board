  ...
  if (mode === 'code' && !html) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-400 p-6 text-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold uppercase tracking-widest mb-1">No Code Provided</p>
          <p className="text-xs">Flip this widget and paste your HTML/CSS/JS code to run it here.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl">
        <button 
          onClick={() => updateWidget(widget.id, { config: { ...widget.config, mode: 'url' } })}
        ...
        <button 
          onClick={() => updateWidget(widget.id, { config: { ...widget.config, mode: 'code' } })}
      ...
      {mode === 'url' ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div>
            ...
            <p className="mt-2 text-[10px] text-slate-400 leading-relaxed italic">
              Pro-tip: Links from YouTube, Google Docs, Slides, and Sheets are automatically formatted.
            </p>
          </div>
          ...
          <div className="p-3 bg-amber-50 border border-amber-100 rounded-lg flex gap-3">
            ...
            <p className="text-[10px] text-amber-800 leading-relaxed font-medium">
              Some websites prevent embedding for security. If the widget is blank, try a different link or document.
            </p>
          </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-2 block tracking-widest">HTML / CSS / JS</label>
            <textarea
              value={html}
              placeholder="<html>&#10;  <style>body { background: #f0f; }</style>&#10;  <body><h1>Hello Class!</h1></body>&#10;</html>"
              onChange={(e) => updateWidget(widget.id, { config: { ...widget.config, html: e.target.value } })}
          ...
          <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg flex gap-3">
            ...
            <p className="text-[10px] text-indigo-800 leading-relaxed font-medium">
              You can paste a full single-file mini-app here. Scripts are allowed but run in a secure sandbox.
            </p>
          </div>
        </div>
      )}
