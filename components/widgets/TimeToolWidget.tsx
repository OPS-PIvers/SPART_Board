...
export const TimeToolWidget: React.FC<Props> = ({ widget }) => {
  ...
  return (
    ...
    >
      ...
      {/* Mode Tabs */}
      <div className="px-8 mt-4 flex gap-4 border-b border-white/5 shrink-0" role="tablist">
        <button
          role="tab"
          ...
          <div className="flex gap-2 mt-4">
            {[60, 300, 600, 900, 1800].map((s) => (
              <button
                key={s}
                onClick={() => setTime(s)}
                className="px-3 py-1 text-xxs  bg-slate-400/10 rounded-lg hover:bg-slate-400/20 transition-colors"
              >
                {s >= 60 ? `${s / 60}m` : `${s}s`}
              </button>
            ))}
