...
**Status:** Phase 4 **code-complete** on `dev-paul`. Invite CFs (`createOrganizationInvites` + `claimOrganizationInvite`), the `organizationMembersSync` onWrite trigger, CSV parser, `/invite/:token` route, and the teacher backfill script all landed in a single review-ready commit. 36/36 unit tests green on touched files; full suite still 1358/1358 green (no regressions). Awaiting CI function deploy on the next `dev-paul` push → Paul's manual QA (task J). The `org-admin-writes` feature flag stays `beta` (paul.ivers only) through this phase; graduation is an explicit separate commit.
...
| Field               | Value                                                                                                                                                                                                                                                                                                                                                                |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Active phase        | Phase 4 **code-complete** — implementation landed on `dev-paul`; awaiting function deploy + Paul's manual QA (I/J) before Phase 4.1                                                                                                                                                                                                                                  |
| Active branch       | `dev-paul` (Phase 4 landed directly; single review-ready commit pending)                                                                                                                                                                                                                                                                                             |
| Last completed task | Phase 4 / H — rules-unit tests extended (invitations locked across every client role; `uid`-write rejection on teacher/building admin). Integration wiring done: CFs exported, `inviteMembers` + `bulkInviteMembers` call the real callable, UsersView CSV modal wired to `parseInvitesCsv` + `createOrganizationInvites`. 36/36 unit tests for touched files green. |
| Last updated (UTC)  | 2026-04-19                                                                                                                                                                                                                                                                                                                                                           |
| Next action         | Phase 4 / I — `firebase deploy --only functions --project spartboard` (auto-fires on `dev-paul` push via `firebase-dev-deploy.yml`). Then Paul runs task J (manual QA: invite → sign in → verify admin write-through + regression on pre-existing admins).                                                                                                           |
...
**Status:** In progress — decisions locked 2026-04-19 (see Decisions Log). Implementation starting with parallel batch 1.
...
- [x] **A2 — Backfill script** (`scripts/backfill-org-members.js`): upsert existing `/users/{uid}` into `members/{emailLower}` as `teacher`. Idempotent, `--dry-run` flag, ADC fallback. Fills Users view for ~90 existing teachers. **Not yet executed — that's a Paul-owned one-shot.**
...
- [ ] **I — Deploy functions to prod.** **User-owned step.** `firebase deploy --only functions --project spartboard` once the PR is merged. CI workflow already auto-deploys on `dev-paul` push, so the merge to `dev-paul` also deploys functions. Rules already Phase-4-ready from Phase 3.
- [ ] **J — Paul manual QA.** Send invite to alt email → open link → sign in → verify member doc + `/admins/{email}` (domain admin role) + `useAuth().isAdmin` === true. Also: flip a member's role from domain_admin → teacher and confirm `/admins/{email}` is removed; repeat on a pre-existing admin and confirm the CF leaves it alone.
- [ ] **K — Graduate `org-admin-writes` flag** to `accessLevel: 'admin'`. **Explicitly deferred** — see Decisions block. Lands as a separate one-line commit after J passes.
- [x] **L — Update this doc.** Phase 4 task ledger closed; Current State advanced to "code-complete, awaiting deploy + QA (I/J)".
