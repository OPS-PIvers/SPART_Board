...
| Next action         | (1) Paul runs `node scripts/backfill-org-members.js --dry-run` on a host with ADC, then without `--dry-run` to populate ~90 teacher member docs. (2) Paul executes task J manual QA per the step-by-step section below. (3) If J passes, land task K (flag graduation) as a separate single-line commit. |
...
- **Counter-maintenance Cloud Function.** `users` on `/organizations/{orgId}`, `/buildings/{id}`, `/domains/{id}` currently reads 0 because it's a denormalized count and nothing maintains it. Needs an onWrite trigger on `members/{emailLower}` that increments/decrements these counters atomically. **Why deferred:** stale display isn't a regression; keeps Phase 4 scope tighter. **Size:** ~half day (trigger + unit test + idempotency for the catch-up run). **Trigger:** Users view looking too obviously broken.
...
- **Teacher backfill run.** `scripts/backfill-org-members.js` exists and is idempotent but has not been executed against prod. **Why deferred:** Paul's call on when to run it — ideally after task J confirms invite flow is solid so any ClassLink-building-id mismatches surface in a predictable context. **Size:** one command (`node scripts/backfill-org-members.js --dry-run` then without the flag). **Trigger:** ready to see all ~90 teachers in the Users view with real counts.
...
- **ClassLink building id alignment.** `hooks/useRosters.ts` stores `buildingId` in `userProfile`, but those ids come from ClassLink, not from `/organizations/{orgId}/buildings`. Will they match? If not, we need a `classLinkBuildingId` alias on `BuildingRecord`. **Status:** deferred — the Phase 4 `backfill-org-members.js` script surfaces this if it runs into mismatches; Paul will call out mismatches during task J's building-assignment spot checks.
