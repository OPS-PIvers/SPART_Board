...
- Comments processed: **5 total — 3 fixed, 2 explained.** All five were `claude[bot]` inline review threads on #2937; the other eight PRs had zero review threads and zero unresolved comments. **This ends a seven-run streak of empty Phase 1s.**
- Fixes pushed: **1** — #2937 / `nightly/widgets-2026-09-09` / commit `5810ca0`: guard `List.tsx` row identity against primitive rows, and cover the sortable-reorder path that no test reached.
