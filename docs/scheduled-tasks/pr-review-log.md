...
- Notes:
  ...
  - Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/diff/review operations used `mcp__github__*` equivalents. Skill files were read from the in-repo `.claude/skills/` (`new-widget`, `admin-widget-config`) — the task's `/mnt/skills/user/` paths do not exist here. Diffs were read locally via `git diff origin/dev-paul...origin/<head>` after `git fetch origin 'refs/heads/*:refs/remotes/origin/*'`; #2908's docs diff again exceeded the inline output limit and was read via a filtered `grep '^+'`.
