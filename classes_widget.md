Implementation Plan: Global Class Roster SystemContextWe are implementing a global "Class Roster" system that persists across all dashboards for a user.Global Data: Rosters are stored in users/{uid}/rosters in Firestore.Dock Integration: A new "Classes" icon in the Dock opens a quick Popout Menu for switching active classes.Classes Widget: A full-featured widget for editing rosters, featuring Smart Paste (auto-splitting First/Last names).Step 1: Type DefinitionsWe need to define the Roster data shape and update the Widget definitions.types.tsAction: Append/Update these types.// ... existing types ...
